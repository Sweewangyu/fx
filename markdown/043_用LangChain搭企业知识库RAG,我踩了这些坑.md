# 用LangChain搭企业知识库RAG，我踩了这些坑

最近我用 LangChain 做了一个偏“业务知识库”风格的 RAG 系统（代码库放在文章最后），不是泛搜索，也不是演示版聊天机器人，而是更贴近企业内部常见的客服 SOP、退款审批规则、OpenAPI 对接手册这类文档问答。
...
为什么单靠“向量检索 + 大模型回答”远远不够

我在实际实现里踩过哪些坑，又是怎么解决的

## 01 系统架构

先看整体架构图：

![image](../assets/043_用LangChain搭企业知识库RAG,我踩了这些坑/img_002.jpg)
...
dense_results =self._dense_search(session_id, query, candidate_k)keyword_results =self._keyword_search(session_id, query, candidate_k)merged_results =self._merge_results(dense_results, keyword_results, candidate_k)returnself._rerank_results(query, merged_results, top_k)

这一步非常值，因为业务文档问答往往既需要语义理解，也需要精确匹配。

## 04 为什么还要再加 Cross-Encoder reranking
...
returnself.reranker.rerank(query, candidates, top_k)
...
classStructuredAnswer(BaseModel):answer: strgrounded: boolcitations: List[Citation] =Field(default_factory=list)
...
## 06 缓存优化：我为什么接了 InMemoryCache

在调 Prompt、反复跑相同问题、反复回归 Benchmark 的时候，最浪费时间的其实是重复调用模型。

所以我加了一层 LangChain InMemoryCache：

fromlangchain_core.cachesimportInMemoryCachefromlangchain_core.globalsimportset_llm_cacheset_llm_cache(InMemoryCache())
...
作者：Wilson Zhang，已获作者授权发布

来源：https://zhuanlan.zhihu.com/p/2016622738203248179
