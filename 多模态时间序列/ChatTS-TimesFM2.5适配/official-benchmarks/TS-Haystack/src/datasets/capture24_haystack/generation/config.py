...
from src.datasets.capture24_haystack.core.data_structures import DifficultyConfig
...
class TaskDifficultyConfig:
    ...
    # Optional absolute needle length range in ms. When set, overrides
    # ``needle_length_ratio_range`` downstream so needles are a fixed size
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "TaskDifficultyConfig":
        ...
        if ms_range is not None:
            if not isinstance(ms_range, (list, tuple)) or len(ms_range) != 2:
                raise ValueError(
                    f"needle_length_ms_range must be a [min_ms, max_ms] pair, "
                    f"got: {ms_range!r}"
                )
            ms_range = (int(ms_range[0]), int(ms_range[1]))
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.needle_length_ms_range is not None:
            out["needle_length_ms_range"] = list(self.needle_length_ms_range)
        return out
...
class GenerationConfig:
    ...
    def get_difficulty_config(
        ...
        return DifficultyConfig(
            ...
            needle_length_ms_range=task_cfg.needle_length_ms_range,
