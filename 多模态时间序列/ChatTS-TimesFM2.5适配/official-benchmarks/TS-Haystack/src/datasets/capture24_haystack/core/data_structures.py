...
class DifficultyConfig:
    ...
    # Absolute needle length range in ms. When set, overrides
    # ``needle_length_ratio_range`` so needle size is independent of context
    # length — for ablating the proportional-needle confound against
    # natural-window haystacks (LTAF, Sleep-PSG).
    needle_length_ms_range: Optional[Tuple[int, int]] = None
    ...
    def get_needle_length_range_samples(self, source_hz: int = 100) -> Tuple[int, int]:
        """
        Compute needle length range in samples.

        If ``needle_length_ms_range`` is set, use the absolute ms range
        (context-length independent). Otherwise fall back to
        ``needle_length_ratio_range`` * context_length_samples.

        Args:
            source_hz: Source data sampling frequency in Hz (only used when
                resolving an absolute ms range).

        Returns:
            Tuple of (min_samples, max_samples) for needle length
        """
        if self.needle_length_ms_range is not None:
            min_ms, max_ms = self.needle_length_ms_range
            return (
                int(min_ms * source_hz / 1000),
                int(max_ms * source_hz / 1000),
            )
    ...
    def get_needle_length_range_ms(self, source_hz: int = 100) -> Tuple[int, int]:
        """
        Compute needle length range in milliseconds.

        Returns the absolute range directly when set; otherwise converts from
        the ratio-based sample range.
        ...
        """
        if self.needle_length_ms_range is not None:
            return (int(self.needle_length_ms_range[0]), int(self.needle_length_ms_range[1]))
        min_samples, max_samples = self.get_needle_length_range_samples(source_hz)
        return (
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "needle_length_ms_range": (
                list(self.needle_length_ms_range)
                if self.needle_length_ms_range is not None else None
            ),
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "DifficultyConfig":
        """Create from dictionary."""
        ms_range = d.get("needle_length_ms_range")
        if ms_range is not None:
            ms_range = tuple(ms_range)
        return cls(
            ...
            needle_length_ms_range=ms_range,
