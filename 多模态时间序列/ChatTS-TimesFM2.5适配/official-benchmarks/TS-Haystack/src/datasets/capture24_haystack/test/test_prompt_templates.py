...
class TestBasicSampling:
    ...
    def test_sample_ordering_boolean_task(self, template_bank, rng):
        """Test sampling ordering boolean task templates."""
        question, answer = template_bank.sample(
            "ordering_boolean",
            rng,
            activity_a="walking",
    ...
    def test_sample_ordering_category_task(self, template_bank, rng):
        """Test sampling ordering category task templates."""
        question, answer = template_bank.sample(
            "ordering_category",
            rng,
            activity_a="walking",
        ...
        assert "walking" in question or "running" in question
        # Category templates should never produce yes/no answers
        assert "Yes" not in answer and "No" not in answer
...
class TestTemplateCounts:
    ...
    def test_sufficient_template_diversity(self, template_bank):
        """Test that each task has sufficient template diversity (10+)."""
        counts = template_bank.get_all_template_counts()

        for task, count in counts.items():
            assert count >= 10, f"Task '{task}' has only {count} templates (expected 10+)"
