...
from src.datasets.capture24_haystack.utils import (
    format_context_dir,
...
class BaseTaskGenerator(ABC):
    ...
    def save_dataset(
        ...
        # Stack signals into (N, 3, L) float32 and save as a sidecar memmap.
        # This mirrors the LTAF / Sleep-PSG loader pattern: parquet holds only
        # the QA metadata, signals are hydrated via np.memmap at access time.
        # Eliminates the per-row parquet deserialization cost for long windows.
        signals = np.stack(
            [
                np.stack([s.x for s in samples], axis=0),
                np.stack([s.y for s in samples], axis=0),
                np.stack([s.z for s in samples], axis=0),
            ],
            axis=1,
        ).astype(np.float32, copy=False)  # (N, 3, L)

        # Build parquet records WITHOUT the signal arrays.
        records = []
        for sample in samples:
            record = sample.to_dict()
            record.pop("x_axis", None)
            record.pop("y_axis", None)
            record.pop("z_axis", None)
        ...
        np.save(task_dir / "signals.npy", signals, allow_pickle=False)
