...
from src.datasets.capture24.loader import (
    CAPTURE24_DATA_DIR,
    load_label_mappings,
)
from src.datasets.capture24.windows import (
    format_window_size,
    get_windows_path,
    load_windows,
)
...
def get_numpy_cache_dir(
    window_size_s: float,
    effective_hz: int,
    label_scheme: str,
    split: str,
) -> Path:
    """Get the path for the numpy memory-map cache directory."""
    dataset_path = get_classification_path(window_size_s, effective_hz, label_scheme)
    return dataset_path / split / "numpy_cache"


def ensure_numpy_cache(
    window_size_s: float,
    effective_hz: int,
    label_scheme: str,
    split: str,
) -> Path:
    """Convert parquet classification data to contiguous numpy arrays for memory-mapping.

    Creates .npy files (x, y, z as float32, labels as int32) next to the parquet file.
    These can be loaded with np.load(path, mmap_mode='r') for near-zero RAM usage.

    Returns:
        Path to the numpy cache directory.
    """
    import numpy as np

    cache_dir = get_numpy_cache_dir(window_size_s, effective_hz, label_scheme, split)
    ...
    print(f"  Building numpy cache for {window_size_s}s/{split} ...")
    df = load_classification_dataset(window_size_s, effective_hz, label_scheme, split)
    n = len(df)

    # All windows within a given size have the same length
    sample_len = len(df["x"][0])

    for col in ("x", "y", "z"):
        arr = np.empty((n, sample_len), dtype=np.float32)
        for i, row in enumerate(df[col]):
            arr[i] = row.to_numpy().astype(np.float32)
        np.save(cache_dir / f"{col}.npy", arr)
        del arr

    labels = df["label_id"].to_numpy().astype(np.int32)
    np.save(cache_dir / "labels.npy", labels)

    del df

    # Atomic sentinel so partial caches are detected
    sentinel.touch()
    print(f"  Numpy cache ready: {cache_dir}  ({n:,} rows, {sample_len} samples/window)")
    return cache_dir
