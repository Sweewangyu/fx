...
"""Parquet loader for LTAF-Haystack runtime datasets."""
...
LTAF_HAYSTACK_TASKS_DIR = Path(RAW_DATA) / "ltafdb" / "ltaf_haystack" / "rhythms" / "tasks"
LTAF_HAYSTACK_COT_DIR = Path(RAW_DATA) / "ltafdb" / "ltaf_haystack" / "rhythms" / "cot"


def _get_data_dir(use_cot: bool, base_dir: Path | None = None) -> Path:
    if base_dir is not None:
        return Path(base_dir)
    return LTAF_HAYSTACK_COT_DIR if use_cot else LTAF_HAYSTACK_TASKS_DIR
...
def _load_split(paths: Iterable[Path]) -> HFDataset:
    # Skip empty shards — the hybrid generator materializes a zero-row
    # parquet per (task, split) combo for coverage enumeration, but HF
    # Dataset.from_parquet errors when one of the input files is empty.
    ...
    for p in paths:
        ...
        except Exception:  # noqa: BLE001 — any parquet-read failure means skip
            continue
    ...
    if not existing:
        return _empty_split()
    return HFDataset.from_parquet(existing, keep_in_memory=False, on_bad_files="error")


def get_available_tasks(base_dir: Path | None = None) -> list[str]:
    root = _get_data_dir(use_cot=False, base_dir=base_dir)
    if not root.exists():
        return []
