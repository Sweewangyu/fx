...
DATASET_VERIFIER="${DATASET_VERIFIER:-${PROJECT_ROOT}/scripts/verify_dataset_snapshot.py}"
...
DATASET_VERIFIER="$(resolve_path "$DATASET_VERIFIER")"
...
if os.environ["TRAINING_RECIPE_HASH"]:
    expected["training_recipe_hash"] = os.environ["TRAINING_RECIPE_HASH"]
else:
    expected.update(
        {
            "trial_id": os.environ["TRIAL_ID"],
            "trial_config_hash": os.environ["TRIAL_CONFIG_HASH"],
            "resolved_configuration_sha256": os.environ[
    ...
    expected.update(
        {
            "trial_config_hash": os.environ["TRIAL_CONFIG_HASH"],
            "resolved_configuration_sha256": os.environ[
                "STAGE1_RESOLVED_CONFIGURATION_SHA256"
            ],
        }
    )
for key, value in expected.items():
...
payload = {
    ...
    "trial_id": os.environ["TRIAL_ID"],
    "trial_config_hash": os.environ["TRIAL_CONFIG_HASH"],
    "training_recipe_hash": os.environ["TRAINING_RECIPE_HASH"],
    "dataset_snapshot_hash": os.environ["DATASET_SNAPSHOT_HASH"],
    "data_version": os.environ["DATA_VERSION"],
...
require_file "Dataset snapshot verifier" "$DATASET_VERIFIER"
...
"$PYTHON_BIN" "$DATASET_VERIFIER" \
    --dataset-dir "$DATASET_DIR" \
    --stage1-datasets "$STAGE1_DATASETS" \
    --stage2-datasets "$STAGE2_DATASETS" \
    --expected-snapshot-hash "$DATASET_SNAPSHOT_HASH" \
