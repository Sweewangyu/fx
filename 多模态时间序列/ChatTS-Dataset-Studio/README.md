...
Studio FIFO，也不受正在运行的 Docker 或其他 Slurm 任务阻塞：每次点击都会立即调用
`sbatch`，GPU 是否已满、何时从 `PENDING` 进入 `RUNNING` 完全由集群调度器决定，页面通过
`squeue/sacct` 展示真实状态和日志。两类任务都会在提交瞬间冻结 resolved YAML、数据快照 Hash
和运行档案；Studio 重启后也会分别恢复 Docker 队列及所有 Slurm tracker。
