...
######################## Embedded Chronos-2 TS Embedding #####################
class ExternalTimeSeriesProjector(nn.Module):
    """The same two-linear-layer projector used by ChatTS-Training."""

    def __init__(self, input_hidden_size: int, llm_hidden_size: int):
        super().__init__()
        ...
        # Do not call this helper ``_init_weights``: Transformers treats any
        # child exposing that name as a nested PreTrainedModel during post_init.
        self.apply(self._initialize_projector_weights)

    @staticmethod
    def _initialize_projector_weights(module):
...
class Chronos2TimeSeriesEmbedding(nn.Module):
    """Frozen, registered Chronos-2 backbone plus the trained projector.

    Unlike the training-time non-Module holder, ``backbone`` is registered so
    ``save_pretrained`` and downstream weight loaders see
    ``ts_encoder.backbone.*``. The backbone is instantiated only from the
    nested config; no model path or network request is used here.
    """

    def __init__(self, config):
        super().__init__()
        if not getattr(config, "chronos2_embedded", False):
            raise ValueError("Chronos-2 requires chronos2_embedded=true in config.json.")

        try:
            from chronos.chronos2 import Chronos2CoreConfig, Chronos2Model
        except ImportError as exc:
            raise ImportError(
                "Install `chronos-forecasting>=2.3.1` to construct the embedded "
                "Chronos-2 module. A separate Chronos checkpoint is not required."
            ) from exc

        ts_config = config.ts
        self.num_features = int(ts_config.get("num_features", 2))
        if self.num_features < 2:
            raise ValueError("Chronos-2 requires ChatTS value and valid-mask features.")

        chronos_config = Chronos2CoreConfig.from_dict(dict(config.chronos2_config))
        forecasting_config = chronos_config.chronos_config
        self.patch_size = int(forecasting_config["input_patch_size"])
        self.context_limit = int(forecasting_config["context_length"])
        chronos_hidden_size = int(chronos_config.d_model)
        projector_input_size = int(config.projector_config["input_hidden_size"])
        if projector_input_size != chronos_hidden_size:
            raise ValueError(
                "Chronos-2/projector dimension mismatch: "
                f"{chronos_hidden_size} != {projector_input_size}."
            )

        self.hidden_size = int(config.hidden_size)
        self.backbone = Chronos2Model(chronos_config)
        self.backbone.requires_grad_(False)
        self.backbone.eval()
        self.projector = ExternalTimeSeriesProjector(projector_input_size, self.hidden_size)
    ...
    def _get_backbone(self, device: torch.device):
        parameter = next(self.backbone.parameters())
        if parameter.device != device or parameter.dtype != torch.float32:
            self.backbone.to(device=device, dtype=torch.float32)
        self.backbone.eval()
        self.backbone.requires_grad_(False)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if torch.any(valid_lengths == 0):
            raise ValueError("Chronos-2 received an empty time series.")
        ...
        with torch.no_grad(), autocast_context:
            for index, expected_count in enumerate(patch_cnt.tolist()):
                context = values[index][valid_mask[index]].to(
                ...
                if returned_count != expected_count:
                    raise RuntimeError(
                        "Chronos-2 patch-count mismatch: "
                        f"returned {returned_count}, expected {expected_count}."
                    )
                context_features.append(
                    encoder_outputs.last_hidden_state[:, :expected_count].squeeze(0)
                )
...
class Qwen3TSForCausalLM(Qwen3TSPreTrainedModel, Qwen3TSGenerationMixin):
    ...
    def __init__(self, config):
        ...
        # TS embedding. Native ChatTS remains available for old checkpoints;
        # this self-contained checkpoint selects Chronos-2 through config.json.
        ts_encoder_type = getattr(config, "ts_encoder_type", "native")
        if ts_encoder_type == "chronos2":
            self.ts_encoder = Chronos2TimeSeriesEmbedding(config)
        elif ts_encoder_type == "native":
            self.ts_encoder = TimeSeriesEmbedding(config.ts)
        else:
            raise ValueError(f"Unsupported ts_encoder_type: {ts_encoder_type!r}.")
